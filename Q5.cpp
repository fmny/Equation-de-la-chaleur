...
int main(int argc , char** argv )
{
 ...
 for (int k=0;k<Th.nv;k++)
    {
      Vertex & v = Th(k);
      if (v.lab==1) 	{x[k]=us; xr[k]=0.; x0[k]=us;  }  
      if (v.lab==3) 	{x[k]=un; xr[k]=0.; x0[k]=un;  }
    } 
  ...
  bool converge1=GradienConjugue(Ar,C1,br,xr,N,1e-10);       
  bool converge0=GradienConjugue(A0,C1,b0,x0,N,1e-10);       
  ...
  if (converge1)
    { // resultat pour gnuplot 
      savesplot(Th,"xr",xr);
      ofstream fsolr("xr.sol");
      fsolr << xr; 
      gnuplot_iso("isor", Th, xr , 25);
    } 
  else 
    {
      cerr << "Erreur : non convergence du gradient conjuge " << endl;
 ...
 if (converge0)
    { // resultat pour gnuplot 
      savesplot(Th,"x0",x0);
      ofstream fsol0("x0.sol");
      fsol0 << x0; 
      gnuplot_iso("iso0", Th, x0 , 25);
    } 
  else 
    {
      cerr << "Erreur : non convergence du gradient conjuge " << endl;
 ...
 if ( converge0*converge*converge1 )
   { // resultat pour gnuplot 
     savesplot(Th,"xx",u);
     ofstream fsoltot("xx.sol");
     fsoltot << u; 
     gnuplot_iso("isotot", Th, u , 25);
   } 
 else 
   {
     cerr << "Erreur : non convergence du gradient conjuge " << endl;
